...
function dialogue_on_insert_comment ($comment)
{
  ...
  /* We should not add the new text proposal if the `new_text_check'
   * checkbox is not checked */
  if (isset ($_POST['new_text_check']))
    $new_text = trim($_POST['dialogue_comment_new_text']);
  else
    $new_text = null;

  /* This updates the just inserted comment with the selected
   * paragraph id. */
  add_comment_meta ($comment, DIALOGUE_CF_NEWTEXTPROPOSAL, $new_text, true) or
    update_comment_meta ($comment, DIALOGUE_CF_NEWTEXTPROPOSAL, $new_text);
  ...
  /* Handling tags */
  if (isset($_POST['comment_tags']) && trim ($_POST['comment_tags']) == '')
    return;

  $tags = explode (',', $_POST['comment_tags']);
  foreach ($tags as $tag)
    {
      ...
      if ($tid == null)
        {
          ...
          $wpdb->insert ($wpdb->prefix . 'dialogue_comment_tags', array ('name' => $name));
          $tid = $wpdb->insert_id;
        }
